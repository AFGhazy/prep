...
void sortingPermutationGeneration() {
    vector<int> v = {5, 7, 1};
    vector<int> t = {0, 1, 2};
...
public:
    int threeSumClosest(vector<int>& nums, int target) {
        ...
        for(int i = 0; i < n; ++i) {
            ...
            while(j < k) {
                if(abs(cur - nums[j] - nums[k]) < abs(diff)) diff = cur - nums[j] - nums[k];
                
                if(cur > nums[j] + nums[k]) {
                    j++;
                } else if(cur < nums[j] + nums[k]) {
                    k--;
                } else {
                    j++;   
                }
...
public:
    vector<int> productExceptSelf(vector<int>& nums) {
        ...
        for(int i = 0; i < n; ++i) {
            m1[i] = i ? m1[i - 1] * nums[i - 1] : 1;
            m2[n - i - 1] = i ? m2[n - i] * nums[n - i] : 1;
        }
        for(int i = 0; i < n; ++i) res[i] = m1[i] * m2[i];
...
public:
    int missingNumber(vector<int>& a) {
        ...
        for(int i = 0; i < n; ++i) if(a[i] < n) return i;
        return n;
    }
};
...
public:
    int trap(vector<int>& h) {
        int n = h.size();
        vector<int> mx(h.size());
        for(int i = n - 1; i >= 0; --i) {
            mx[i] = (i < n - 1 ? max(mx[i + 1], h[i + 1]) : 0);
        }
        int tot = 0;
        int mxSoFar = 0;
        for(int i = 0; i < n; ++i) {
            int mn = min(mxSoFar, mx[i]);
            if(mn > h[i])
                tot += mn - h[i];
            mxSoFar = max(mxSoFar, h[i]);
        }
        return tot;
    }
};

class SolutionPrisonAfterNDays {
public:    
...
class SolutionMaxDistToClosest {
public:
    int maxDistToClosest(vector<int>& seats) {
        ...
        vector<int> df(n, 0), db(n + 1, 0);
        for(int i = 0; i < n; ++i) {
            df[i] = i ? max(df[i - 1], seats[i - 1] ? i : -inf) : -inf;
        }
    
        for(int i = n - 1; i >= 0; --i) {
            db[i] = i < n - 1 ? min(db[i + 1], seats[i + 1] ? i : inf) : inf;
        }
        
        int mx = 0;
        for(int i = 0; i < n; ++i) {
            if(!seats[i])
            mx = max(mx, min(i - df[i], db[i] - i) + 1);
        }
...
class SolutionMaxDistToClosestOpt {
    int maxDistToClosest(vector<int> seats) {
        int left = -1, maxDis = 0;
        int len = seats.size();
